...
public class RespostaDAO extends BDOpenHelper {
    ...
    public long insertResposta(Resposta resposta){
        ...
        values.put(ID_QUESTAO, resposta.getQuestao().getId_questao());
        values.put(ID_QUESTIONARIO, resposta.getQuestionario().getId_questionario());
        values.put(OPCAO_RESPOTA, resposta.getOpcao());
        values.put(NOME_PROF_SERV, resposta.getNomeProfServ());
        values.put(ENDERECO_RESPOSTA, resposta.getEndereco());

        long id = db.insert(TABELA_RESPOSTA, null, values);
        db.close();
    ...
    private Resposta cursorToResposta(Cursor cursor) {
        ...
        long id_questao = cursor.getLong(cursor.getColumnIndex("id_questao"));
        QuestaoDAO questaoDAO = new QuestaoDAO(this.context);
        Questao questao = questaoDAO.getQuestaoById(id_questao);
