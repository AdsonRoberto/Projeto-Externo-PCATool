...
import br.uece.gesad.pcatoolbrasil.model.domain.Posto;
...
public class PostoDAO extends BDOpenHelper {
    ...
    public long insertPosto(Posto posto){
        ...
        values.put(ID_REGIONAL, posto.getRegional().getId_regional());
        values.put(NOME_POSTO, posto.getNome());
        values.put(ENDERECO_POSTO, posto.getEndereco());

        long id = db.insert(TABELA_POSTO, null, values);
        db.close();
